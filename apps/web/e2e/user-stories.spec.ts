...
test.describe("Major User Stories", () => {
  test("New User: should see the initial scaffold and be able to start", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    await expect(
      page.getByText("Select or create a sketch, or select a space to view."),
    ).toBeVisible();
  });

  test("User Story: Create a simple Sketch and Fill Space", async ({
    page,
  }) => {
    await page.goto("/");
