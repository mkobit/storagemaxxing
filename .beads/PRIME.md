...
- **`bd show <epic>` reporting "N/N complete (100%) -- eligible for close" does NOT mean the linked OpenSpec change is actually done.** It only reflects child-bead completion -- it says nothing about whether `retrospective.md`/`reflection.md` still need writing (see the mandatory-retrospective rule above). Confirmed: `bd show sm-vd2g` reported 100% with both artifacts still outstanding; the gap only surfaced via a separate `bunx openspec status --change <name> --json` call (sm-xgsq). Before closing/archiving any epic carrying a `meta:openspec:<name>` label, always cross-check `bunx openspec status --change <name> --json` too -- don't trust bead-completion percentage alone.
- **`feature-probe`-originated changes used to reach `human-review` with no `tasks.md` at all**, because none of its 5 steps ever authored one -- `remediation-plan`'s own description said "update ... tasks.md", implying it already existed. `openspec-sync`'s `expand-tasks` step only parses existing checkboxes, so it had nothing to hydrate from (sm-nqkd, hit twice on sm-mol-bhbn/unattended-agent-sandbox and sm-mol-xeus/ci-quality-gates). Fixed by inserting a `draft-tasks` step between `remediation-plan` and `human-review`. Any feature-probe molecule poured _before_ this fix landed may still be missing the step -- check `bd show <human-review-id>` for a sibling `draft-tasks` bead before assuming `openspec-sync` has something to hydrate.
- **`gh api repos/.../branches/main/protection` returns 404 'Branch not protected' on repos using GitHub Rulesets.** A 404 does not mean branch protection or required status checks are absent -- it only means classic branch protection is not in use. When verifying whether renaming/splitting a CI job is safe, query both `gh api repos/.../branches/main/protection` AND `gh api repos/.../rulesets` before concluding no required check references the old job name (sm-gbi7).
- **Parallel Agent spawns that mutate `bd` state (`bd mol pour`, `bd create`) against shared parent state can race and duplicate hydration.** If a spawn appears to hang or crash and is relaunched, the original subagent might still be alive and execute `bd mol pour`, creating duplicate child trees. When planning parallel subagent spawns that mutate `bd` state, either sequence those `bd`-mutating steps or instruct each agent to verify existing children (`bd show <id>`) before pouring a molecule (sm-dmj5).
