...
- **The `git-commit-main-guard` PreToolUse hook (`.agents/hooks/git-commit-main-guard.ts`) matches the raw Bash command *string*, not an actual git invocation.** Any command whose text happens to contain "git" and "commit" adjacently -- e.g. inside a quoted `bd create`/`bd close --reason`/`--description` string that's *describing* the guard, not running it -- gets blocked with a misleading branch-protection error, even on a harmless `bd` command. Hit repeatedly while filing beads about this exact hook (sm-brim tracks a proper fix, still open). Workaround until fixed: reword the string so the two words aren't adjacent (e.g. hyphenate as "git-commit").
