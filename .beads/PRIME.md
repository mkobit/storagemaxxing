...
- **The `git-commit-main-guard` PreToolUse hook (`.agents/hooks/git-commit-main-guard.ts`) used to match the raw Bash command *string*, not an actual git invocation** -- any command whose text merely contained "git" and "commit" adjacently inside a quoted `bd close --reason`/`--description` string (describing the guard, not running it) got blocked with a misleading branch-protection error. Hit repeatedly while filing beads about this exact hook. Fixed in sm-brim by stripping quoted spans before matching; if a false-positive resurfaces, check whether the trigger text is *unquoted* (the fix only handles quoted spans).
