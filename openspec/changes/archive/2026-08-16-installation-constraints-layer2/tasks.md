...
- [x] 1. Catalog weight support (`packages/catalog`)
  - Add optional `weightLbs?: number` to `BinSpec` and `BinSpecSchema`.
  - Validation: `bun test packages/catalog`

- [x] 2. Assembly weight overflow failure schema (`packages/assembly`)
...
- [x] 3. Store selector filtering & layout resolution (`packages/store`)
...
- [x] 4. Web UI constraint controls (`apps/web`)
