...
- [x] 1.1 [sm-5j1g](../../../.beads) Wire GitHub Actions step: build `apps/web`, deploy via `bunx wrangler deploy` on push to `main` only
...
- [x] 2.1 [sm-eujc](../../../.beads) Broaden `CLOUDFLARE_API_TOKEN` permissions to include `Workers Scripts:Edit` (or issue a replacement token)
