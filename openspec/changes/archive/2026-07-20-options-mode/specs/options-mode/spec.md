...
For the active space, the application SHALL show one strategy card per comparable storage system (`schaller`, `gridfinity`, `akromils`).
...
- **THEN** up to three cards render — one each for Schaller, Gridfinity, and Akro-Mils — and no card mixes bins from more than one system

#### Scenario: No card is ranked or scored

- **WHEN** the three cards are rendered
