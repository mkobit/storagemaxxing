...
`apps/web/src/ui/AGENTS.md` documents a philosophy of preferring E2E tests over isolated component *tests* — Storybook here is a viewing/development tool, not a test runner, so no existing test moves and none of `ui/AGENTS.md`'s guidance is contradicted.
...
- Replacing or reducing Playwright e2e coverage. `ui/AGENTS.md`'s "prefer E2E over isolated component testing" stance is about *test* strategy and is unchanged.
...
- `main.ts`: `framework: "@storybook/react-vite"`, `stories: ["../src/ui/**/*.stories.tsx"]`. No custom `viteFinal` needed — `@storybook/react-vite` reads `apps/web/vite.config.ts` for the existing plugins/aliases/Tailwind setup by default, so the DAG aliases and `@tailwindcss/vite` plugin apply identically to Storybook and the real app.
- `preview.tsx`: imports `../src/index.css` (loads Tailwind + fonts + tokens) and wraps every story in `ThemeProvider` via a global decorator, matching how `src/index.tsx` mounts `<ThemeProvider><App /></ThemeProvider>`.
- Storybook has no built-in dark-mode toggle (its core `backgrounds` feature only swaps canvas background-color swatches); the standard, widely-documented approach is a custom `globalTypes` entry in `preview.tsx` plus a decorator that reads `context.globals` and toggles the `dark`/`light` class on the Storybook preview iframe's own `<html>` — exercising the same `.dark`-class mechanism `ThemeProvider` uses at runtime, just driven by the toolbar selection instead of `ThemeProvider`'s `localStorage`/`prefers-color-scheme` read path (see Risks below).
