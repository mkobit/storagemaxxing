...
- [x] 1.1 [sm-8shp](../../../.beads) Pin `storybook@10.5.2` + `@storybook/react-vite@10.5.2` to `apps/web` devDependencies
  - Validation: `bun install --frozen-lockfile` resolves cleanly; `bun run --filter @storagemaxxing/web build` (main app) still succeeds.

## 2. Storybook config

- [x] 2.1 [sm-7pvd](../../../.beads) Add `apps/web/.storybook/main.ts` + `preview.tsx`, `storybook`/`build-storybook` scripts
...
- [x] 3.1 [sm-zie9](../../../.beads) Wire `eslint-plugin-storybook` + `apps/web/tsconfig.json` include for Storybook files
...
- [x] 4.1 [sm-ailr](../../../.beads) Add stories: `ThemeToggle`, `StrategyCard`
...
- [x] 4.2 [sm-pmv3](../../../.beads) Add stories: `BOMRow`, `BOMHeader`, `BOMSummary`
...
- [x] 4.3 [sm-ljaw](../../../.beads) Add stories: `ConstraintRow`, `ConstraintInputs`
...
- [x] 5.1 [sm-6vrs](../../../.beads) Add `build-storybook` step to `.github/workflows/ci.yml`'s `verify` job
  - Validation: a deliberately broken story fails the `verify` job's Storybook build step; reverting it passes.

## 6. Verification

- [x] 6.1 [sm-totw](../../../.beads) Full quality-gate verification for `storybook-adoption`
