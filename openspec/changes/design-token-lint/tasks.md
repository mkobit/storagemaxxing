...
- [x] 1.1 [sm-sbqr](../../../.beads) Delete dead `GridVisualizer.tsx` to unblock design-token lint rule
  - Validation: `rg -l GridVisualizer apps/web` returns no matches; `bun run typecheck` and `bun test` pass.
- [x] 1.2 [sm-zo8s](../../../.beads) Replace arbitrary Tailwind bracket values with scale utilities in `ConstraintEditorPanel`/`ConstraintInputs`/`ConstraintRow`
...
- [x] 2.1 [sm-vwdk](../../../.beads) Add design-token lint rules (hex-color + arbitrary-bracket) to `eslint.config.ts`, depends on 1.1 and 1.2
