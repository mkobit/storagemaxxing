...
- ~~Should `mode:hotl` human-approval-before-merge be promoted from a procedural convention to a ruleset-enforced required-reviewer rule immediately, or only after observing procedural compliance fail at least once?~~ **Resolved (human review, sm-mol-bhbn)**: ship procedural first; promote to ruleset-enforced only if procedural compliance fails in practice. The more severe self-merge-from-inside-the-session path is already closed independently (gating `gh pr merge`, Decision 2).
