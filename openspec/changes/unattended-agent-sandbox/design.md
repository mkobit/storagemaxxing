...
- Enforces the egress allowlist from **outside** the sandboxed process's own network namespace (host-level Docker network policy or a proxy sidecar with a separate namespace) — not via `iptables`/`nftables` rules run with `NET_ADMIN` *inside* the container the agent's own tool calls can reach, since that would hand the same untrusted principal the capability to unset its own restriction.
...
- **`mode:hotl` procedural (not ruleset-enforced) human review**: Decision 4 deliberately keeps the human-approval-for-`mode:hotl` rule procedural rather than ruleset-enforced at first, for a *human's* own later merge action. The independent pass's more severe version of this gap — a sandboxed session merging its **own** `mode:hotl` PR from inside the session, bypassing the human step entirely — is closed by gating `gh pr merge` (Decision 2). The procedural-vs-ruleset question for a human's own workflow remains open (see Open Questions).
