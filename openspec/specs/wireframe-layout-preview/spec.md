# wireframe-layout-preview Specification

## Purpose

The wireframe-layout-preview capability lets a user see bin heights relative to each other and to the space's own height, on demand, on the existing layout canvas.
It adds a pure cabinet-projection primitive in `packages/geometry`, a pure scene builder in `apps/web` that turns a `PackingResult` into ordered, projected 2D polygons, and an opt-in toggle that repaints the canvas as an oblique wireframe instead of the default top-down 2D view.
The 2D view remains the default and is byte-for-byte unchanged when the toggle is off.
