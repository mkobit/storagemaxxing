...
export const checkFeasibility = async (request: SolverRequest): Promise<SolverResult> => {
  ...
  type Bnd = { readonly name: string; readonly type: number; readonly lb: number; readonly ub: number };
  type SubjTo = { readonly name: string; readonly vars: readonly Var[]; readonly bnds: { readonly type: number; readonly lb: number; readonly ub: number } };
  ...
  const spaceModelParts = spaces.reduce((acc, space) => {
    ...
    const binModelParts = bins.reduce((binAcc, bin) => {
      ...
      const boundInfo = (() => {
        ...
        if (constraint.mode === "hard") {
            if (constraint.hi !== null) return { lb: constraint.lo, ub: constraint.hi, type: GLP_DB };
            return { lb: constraint.lo, ub: 0, type: GLP_LO };
        }
        if (constraint.mode === "soft") {
            if (constraint.hi !== null) return { lb: 0, ub: constraint.hi, type: GLP_DB };
    ...
    }, { vars: [] as readonly Var[], bounds: [] as readonly Bnd[], geomVars: [] as readonly Var[], generals: [] as readonly string[] });
  ...
  const aggConstraints = aggregateConstraints.filter(ac => ac.hard).reduce((acc, ac, i) => {
      const acVars = spaces.map(space => ({ name: varName(space.id, ac.binId), coef: 1 }));
  ...
  const suggestedCounts = isFeasible && result.result.vars
    ? spaces.reduce((spaceAcc, space) => {
