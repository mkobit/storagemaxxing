...
beforeAll(() => {
  ...
  if (
    ...
    !(globalThis as any).self.location
  ) {
    (globalThis as any).self.location = { href: "http://localhost" };
  }
});
...
describe("feasibility", () => {
  ...
  it("should return feasible for valid constraints", async () => {
    ...
    expect(result.suggestedCounts["space-1"]["bin-1"]).toBeGreaterThanOrEqual(
      2,
    );
    expect(Number.isInteger(result.suggestedCounts["space-1"]["bin-1"])).toBe(
      true,
    );
  });
