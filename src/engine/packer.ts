...
import { SpaceConstraint } from '../assembly/SpaceConstraint';
import { createPlacedBin } from '../assembly/PlacedBin';
import { createPoint3D } from '../geometry/Point3D';
import { PackingResult, ConstraintFailure, createConstraintFailure, createPackingMetrics, createPackingResult, ValidityState } from './types';
import { getEffectiveFootprint, getEffectiveSpaceDimensions, getMaxBinDepth } from './geometryUtils';
...
const checkHardMinPhase = (constraints: readonly SpaceConstraint[], packer: MaxRectsPacker) => {
  const failures = checkPhaseFailures(constraints, packer, (c) => c.hardMin, 'hardMin');
  return { validity: failures.length > 0 ? ('invalid' as const) : ('valid' as const), failures };
};

const checkSoftMinPhase = (constraints: readonly SpaceConstraint[], packer: MaxRectsPacker, validity: ValidityState) => {
  if (validity !== 'valid') return { validity, failures: [] };
  const failures = checkPhaseFailures(constraints, packer, (c) => c.softMin, 'softMin');
  return { validity: failures.length > 0 ? ('partial' as const) : ('valid' as const), failures };
};

const generateAutoFillRects = (constraints: readonly SpaceConstraint[], binMap: ReadonlyMap<string, BinSpec>, spaceArea: number): RectsAccumulator => {
  const unconstrained = constraints.filter((c) => c.max === undefined).map((c) => binMap.get(c.binId)!);
  if (unconstrained.length === 0) return [];
  const sorted = [...unconstrained].sort((a, b) => getEffectiveFootprint(b).w * getEffectiveFootprint(b).l - getEffectiveFootprint(a).w * getEffectiveFootprint(a).l);
...
const executePhases = (
  ...
  const add = (rects: RectsAccumulator) => // @ts-expect-error MaxRectsPacker TS definitions are missing readonly annotations
    packer.addArray(Array.from(rects));

  // eslint-disable-next-line functional/no-expression-statements
  add(sortRects(generatePhaseRects(constraints, binMap, (c) => c.hardMin)));
  const hmCheck = checkHardMinPhase(constraints, packer);

  // eslint-disable-next-line functional/no-expression-statements
  add(sortRects(generatePhaseRects(constraints, binMap, (c) => Math.max(0, c.softMin - c.hardMin))));
  const smCheck = checkSoftMinPhase(constraints, packer, hmCheck.validity);

  // eslint-disable-next-line functional/no-expression-statements
  add(sortRects(generatePhaseRects(constraints, binMap, (c) => (c.max !== undefined && c.max > c.softMin ? c.max - Math.max(c.hardMin, c.softMin) : 0))));
