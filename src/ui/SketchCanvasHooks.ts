...
const handleTwoPointRect = (start: Point, current: Point, addElement: (el: SketchElement) => void) => {
...
export const useSketchEvents = (canvasRef: MutableRefObject<HTMLCanvasElement | null>) => {
  ...
  const handlePointerDown = useCallback((e: MouseEvent<HTMLCanvasElement>) => {
  ...
  const handlePointerMove = useCallback((e: MouseEvent<HTMLCanvasElement>) => {
    if (!isDrawing) return;
    const rect = canvasRef.current?.getBoundingClientRect();
    if (!rect) return;
    const x = e.clientX - rect.left;
  ...
  return { isDrawing, startPoint, currentPoint, handlePointerDown, handlePointerMove, handlePointerUp };
};
